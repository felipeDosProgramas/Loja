...
<script>
    let toTest = /[!]|[-]/,
    inputNome = document.getElementsByName('nome');
    let cronometro = null

    inputNome[0].addEventListener('input', ()=> {
        clearTimeout(cronometro);
        cronometro = setTimeout(()=>{
            if(toTest.test(inputNome[0].value)){
                inputNome[0].value = "";
        }
        }, 100)
    ...
    let spanSucess = document.getElementById('serverResponse');
        function responseHand(){
            switch(response){
                case 'cadastrado':
                    
                    console.log("certo")
                    spanSucess.innerText= "prévia cadastrada com sucesso";
                    spanSucess.style.color = 'rgb(0 255 31)'
                break;
                case 'existe':
                    console.log("tem")
                    spanSucess.innerText = "prévia já existe";
                    spanSucess.style.color = 'red'
                break;
            }
        }
        <?php 
            if(isset($_GET['response'])){echo "var response = '".$_GET['response']."'
